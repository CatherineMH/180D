...
/*************************COLLECT_DATA_TURNS********************************
 * RAW DATA COLLECTION METHOD THAT PROMPTS THE USER WHEN TO TURN
 * AND PUTS AN EXTRA COLUMN IN THE OUTPUT FILE WHERE 1 = IS TURNING, 0 = IS NOT TURNING
 * PURPOSE IS TO DETERMINE EXACTLY WHERE TURNS ARE LOCATED WHEN TRAINING ANN
 *
 * */
...
int main()
{
    ...
    */
    ...
    int collection_duration = 20; //seconds of data collection. Human activity at ~15Hz = 4 seconds/activity
    ...
    while (run_flag)
    {
        ...
        sprintf(file_name,"data_%ld_turn.csv", start_time_sec);
        fp = fopen(file_name, "w");
        if (fp == NULL)
        {
            fprintf(stderr, "Failed to open file \'%s\'. Exiting.\n",
