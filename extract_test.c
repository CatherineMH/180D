  ...
  /*  printf("Opening file %s\n", ifile_name);
    ...
    while ((read = getline(&line, &len, fp)) != -1)
    {
        ...
        for (i = 0; i < 6; i++) {
            if (calc_out[i] > max) {
                max = calc_out[i];
                activity_type = i; // motion =i;
            }
        }
    
        printf(" 0 - Walking, 1 - Upstairs, 2 - Downstairs, 3 - Jump, 4 - Left Turn, 5 - Right Turn. PREDICTED-%d (%f, %f, %f, %f, %f, %f)\n", activity_type,
...
void calculate_correlation_coefficient(float *arr_x, float *arr_y, int n_P, int* index_period_gz, float* correlation)
{
    ...
    //r = (double *) malloc(sizeof(double) * n_P_gz);
    for (i=0; i < n_P-1; i++)
    {
...
//find_index_maxima_gz(ifile_name, N_SAMPLES, gz,t, P_i_gz, n_P_gz)

 void find_index_maxima_gz(const char *ifile_name, int N_SAMPLES, float* gz, double* t, int* new_maxima_idx, int* n_P_gz)
{
    FILE *fp;
    ...
    int *maxima_idx = (int*) malloc(sizeof(float)*N_SAMPLES);
    ...
    int end_count = 200; //a parameter that determines how long we go for before looking for a new maximum
    int idx = 0;
...
void process_file(const char *ifile_name)
{
   ...
   // T_i_gz = (int *) malloc(sizeof(int) * N_SAMPLES);
    /* step4 find number(n_P_gz, n_T_gz) and indexes(P_i_gz, T_i_gz) of peaks and throughs in AS
    local_max_min(
                  gz,
                  AV,
                  N_SAMPLES,
    ...
    */
    ...
    find_index_maxima_gz(ifile_name, N_SAMPLES, gz,t, P_i_gz, &n_P_gz);
    printf("Coing from function find_index_maxima_gz\n");
    
    
    /* step5 find tau (period) and shifted indexes for period start(index_period_gz) based on gz and n_P_gz */
    int k =0;
    /* allocate array for tau (period in sec of each stride) */
    double* period_time_gz = (double *) malloc(sizeof(double) *n_P_gz);
    /* the shift in index of peaks in AS to make period start from zero */
    int index_shift_gz;
    /* an array of final SHIFTED INDEXES for the period */
    int* index_period_gz = (int *) malloc(sizeof(int) *n_P_gz);
    ...
    /* max and min values and arrays based on n_P_gz */
    ...
    max_ar_ax = (float *) malloc(sizeof(float) * n_P_gz);
    min_ar_ax = (float *) malloc(sizeof(float) * n_P_gz);
    ratio_ax = (float *) malloc(sizeof(float) * n_P_gz);
    
    max_ar_ay = (float *) malloc(sizeof(float) * n_P_gz);
    min_ar_ay = (float *) malloc(sizeof(float) * n_P_gz);
    ratio_ay = (float *) malloc(sizeof(float) * n_P_gz);
    
    max_ar_az = (float *) malloc(sizeof(float) * n_P_gz);
    min_ar_az = (float *) malloc(sizeof(float) * n_P_gz);
    ratio_az = (float *) malloc(sizeof(float) * n_P_gz);
    
    max_ar_gx = (float *) malloc(sizeof(float) * n_P_gz);
    min_ar_gx = (float *) malloc(sizeof(float) * n_P_gz);
    ratio_gx = (float *) malloc(sizeof(float) * n_P_gz);
    
    max_ar_gy = (float *) malloc(sizeof(float) * n_P_gz);
    min_ar_gy = (float *) malloc(sizeof(float) * n_P_gz);
    ratio_gy = (float *) malloc(sizeof(float) * n_P_gz);
    
    max_ar_gz = (float *) malloc(sizeof(float) * n_P_gz);
    min_ar_gz = (float *) malloc(sizeof(float) * n_P_gz);
    ratio_gz = (float *) malloc(sizeof(float) * n_P_gz);

    find_max_min_ratio(x, n_P_gz, index_period_gz, max_ar_ax, min_ar_ax, ratio_ax);
    find_max_min_ratio(y, n_P_gz, index_period_gz, max_ar_ay, min_ar_ay, ratio_ay);
    find_max_min_ratio(z, n_P_gz, index_period_gz, max_ar_az, min_ar_az, ratio_az);
    
    find_max_min_ratio(gx, n_P_gz, index_period_gz, max_ar_gx, min_ar_gx, ratio_gx);
    find_max_min_ratio(gy, n_P_gz, index_period_gz, max_ar_gy, min_ar_gy, ratio_gy);
    find_max_min_ratio(gz, n_P_gz, index_period_gz, max_ar_gz, min_ar_gz, ratio_gz);
    
    //mean
    float * mean_ar_ax, * mean_ar_ay;
    float * mean_ar_gy, * mean_ar_gz;
    
    mean_ar_ax = (float *) malloc(sizeof(float) * n_P_gz);
    mean_ar_ay = (float *) malloc(sizeof(float) * n_P_gz);
    mean_ar_gy = (float *) malloc(sizeof(float) * n_P_gz);
    mean_ar_gz = (float *) malloc(sizeof(float) * n_P_gz);
    
    calculate_mean(x, n_P_gz, index_period_gz, mean_ar_ax);
    calculate_mean(y, n_P_gz, index_period_gz, mean_ar_ay);
    calculate_mean(gy, n_P_gz, index_period_gz, mean_ar_gy);
    calculate_mean(gz, n_P_gz, index_period_gz, mean_ar_gz);
    ...
    deviation_ar_ax = (float *) malloc(sizeof(float) * n_P_gz);
    deviation_ar_ay = (float *) malloc(sizeof(float) * n_P_gz);
    deviation_ar_gy = (float *) malloc(sizeof(float) * n_P_gz);
    deviation_ar_gz = (float *) malloc(sizeof(float) * n_P_gz);
    
    variance_ar_ax = (float *) malloc(sizeof(float) * n_P_gz);
    variance_ar_ay = (float *) malloc(sizeof(float) * n_P_gz);
    variance_ar_gy = (float *) malloc(sizeof(float) * n_P_gz);
    variance_ar_gz = (float *) malloc(sizeof(float) * n_P_gz);
    
    
    calculate_deviation(x, n_P_gz, index_period_gz, mean_ar_ax, deviation_ar_ax, variance_ar_ax);
    calculate_deviation(y, n_P_gz, index_period_gz, mean_ar_ay, deviation_ar_ay, variance_ar_ay);
    calculate_deviation(gy, n_P_gz, index_period_gz, mean_ar_gy, deviation_ar_gy, variance_ar_gy);
    calculate_deviation(gz, n_P_gz, index_period_gz, mean_ar_gz, deviation_ar_gz, variance_ar_gz);
   
    //skewness
    float * skewness_ar_ax, * skewness_ar_ay;
    float * skewness_ar_gy, * skewness_ar_gz;
    
    skewness_ar_ax = (float *) malloc(sizeof(float) * n_P_gz);
    skewness_ar_ay = (float *) malloc(sizeof(float) * n_P_gz);
    skewness_ar_gy = (float *) malloc(sizeof(float) * n_P_gz);
    skewness_ar_gz = (float *) malloc(sizeof(float) * n_P_gz);
    
    calculate_skewness(x, n_P_gz, index_period_gz, mean_ar_ax, deviation_ar_ax,skewness_ar_ax);
    calculate_skewness(y, n_P_gz, index_period_gz, mean_ar_ay, deviation_ar_ay,skewness_ar_ay);
    calculate_skewness(gy, n_P_gz, index_period_gz, mean_ar_gy, deviation_ar_gy,skewness_ar_gy);
    calculate_skewness(gz, n_P_gz, index_period_gz, mean_ar_gz, deviation_ar_gz,skewness_ar_gz);

    //kurtosis
    float * kurtosis_ar_ax, * kurtosis_ar_ay;
    float * kurtosis_ar_gy, * kurtosis_ar_gz;
    
    kurtosis_ar_ax = (float *) malloc(sizeof(float) * n_P_gz);
    kurtosis_ar_ay = (float *) malloc(sizeof(float) * n_P_gz);
    kurtosis_ar_gy = (float *) malloc(sizeof(float) * n_P_gz);
    kurtosis_ar_gz = (float *) malloc(sizeof(float) * n_P_gz);
    
    calculate_kurtosis(x, n_P_gz, index_period_gz, mean_ar_ax, deviation_ar_ax,kurtosis_ar_ax);
    calculate_kurtosis(y, n_P_gz, index_period_gz, mean_ar_ay, deviation_ar_ay,kurtosis_ar_ay);
    calculate_kurtosis(gy, n_P_gz, index_period_gz, mean_ar_gy, deviation_ar_gy,kurtosis_ar_gy);
    calculate_kurtosis(gz, n_P_gz, index_period_gz, mean_ar_gz, deviation_ar_gz,kurtosis_ar_gz);
    
    //correlation coefficient
    float * correlation_ar_ax_ay;
    correlation_ar_ax_ay = (float *) malloc(sizeof(float) * n_P_gz);
    calculate_correlation_coefficient(x, y, n_P_gz, index_period_gz, correlation_ar_ax_ay);
    
   /* for (i = 0; i < n_P_gz-1; i++)
    {
        printf("%d,%f,%f,%f,%f,%f\n",
               i,
               period_time_gz[i],
    ...
    */
    ...
    char *out_file = "ann1_training_file.txt";
   ...
   int one = 1;
    ...
    for (i = 0; i < n_P_gz-1; i++)
    {
        ...
        fprintf(fp, "%d %d %d %d %d %d\n", neg_one, neg_one, neg_one, neg_one, neg_one, one);
    }
...
int main()
{
    ...
    process_file(ifile_name);          //train  -1
    //process_file("Walking_Ten_strides_2.csv");         //test   -1
      //process_file("2sensors_upstairs_ankle_1.csv");      //train  -2
    //process_file("2sensors_downstairs_ankle_1.csv");    //train  -3
      //process_file("Jump_10times_2.csv");                 //train  -4
        //process_file("Walk&TurnLeft_5stepsX2_1.csv");        //train  -5
      //process_file("Walk&TurnLeft_5stepsX2_2.csv");        //test  - 5
     // process_file("Walk&TurnRight_5stepsX2_1.csv");       //train - 6
    //process_file("Walk&TurnRight_5stepsX2_2.csv");       //test - 6
    //process_file("jump_data-3.csv"); //test jump

    
    /*while(1)
    {
        //check for files created by producer
        printf("system(\"ls -1 data_149*.csv > files_to_process.txt\");\n");
        system("ls -1 data_149*.csv > files_to_process.txt");
        ...
        while((read = getline(&line, &len, fp) != -1)){
            ...
            // NO FUNCTIONS IN HERE SHOULD BE LOOPING
            ...
            activity_type = global_neural_network("ann1_test_file.txt"); // make sure to delete test file
            // features = process_file(ifile_name);
            // activity_type = neural_network(features); // now I know if its
            // features_2 = process_file_given_activity(activity_type, ifile_name);
            // activity_metrics = neural_network_2(features_2, activity_type);
